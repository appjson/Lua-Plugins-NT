...
function ReceiveFriendMsg(CurrentQQ, data)
    ...
    if not Content:find("^#一言：") and not Content:find("^百度一下：") then
        return 1
    end
    ...
    if accessToken == nil or accessToken == "" then
        ...
        if oldTime == nil or nowTime - oldTime >= 30 * 24 * 60 * 60 then
            UpdateToken()
            LuaApi.Action:sendFriendText(data.FromUin, "Token过期或无效，正在刷新Token。\n请稍后再试")
            return 1
        end
    end
    -- request YiYan
    local content = Content:gsub("#一言：", "")
    content = content:gsub("百度一下：", "", 1)
...
function ReceiveGroupMsg(CurrentQQ, data)
    ...
    if accessToken == nil or accessToken == "" then
        UpdateToken()
        LuaApi.Action:sendGroupText(data.FromGroupId, "Token过期或无效，正在刷新Token。\n请稍后再试")
        return 1
    else
        accessToken = json.decode(accessToken)
        local oldTime = accessToken.time
...
function RequestYiYan(content, accessToken)
    ...
    local option = {
        system = "你是一个帮助用户解决各种问题的百度大语言模型，角色是一个无所不知的问答助理，你的管理员叫小渚。接下来用户会给你提问，你需要根据问题回复答案。请直接回复答案本身即可，不需要重复答案或对答案进行解释。使用纯文本进行输出而不是Markdown。",
        messages = {
            {
                role = "user",
                content = content
